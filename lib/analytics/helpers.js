...
// Drop-off before completing. `via` = how they left: 'navigation' (in-app SPA
// nav) or 'pagehide' (tab close / refresh — best-effort, may not always flush).
export const trackLessonAbandoned = ({ lessonId, category, coachingStyle, via }) => {
  Mixpanel.track(EVENTS.LESSON_ABANDONED, {
    lesson_id: lessonId,
    category,
    coaching_style: coachingStyle,
    via,
...
export const trackRecordingAbandoned = ({ lessonId, category, coachingStyle, via }) => {
  Mixpanel.track(EVENTS.RECORDING_ABANDONED, {
    lesson_id: lessonId,
    category,
    coaching_style: coachingStyle,
    via,
